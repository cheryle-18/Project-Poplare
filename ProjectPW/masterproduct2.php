<?php
    ...
    if($current_cat!="") $stmt = $conn->prepare("SELECT * FROM items WHERE Category = '$current_cat'");
...
<body style="background-color: #f5f3f0;">
    ...
    <div class="content ps-5 pe-5 pt-5 pb-5 mx-auto <?= $current_search==""?"d-block":"d-none"?>" style="justify-content: center;" >
        <div class="cards d-flex flex-wrap justify-content-center" style="margin-top: 70px;">
            <?php
                ...
                for($i=(16*$currPage); $i<$batas; $i++){  
                    $pass = false;
                    if($current_sub!=""){
                        if($items[$i]["Sub_Category"] == $current_sub) $pass = true;
                    }
                    else $pass = true;
                    if($pass){
    ...
    <div class="content ps-5 pe-5 pt-5 pb-5 mx-auto <?= $current_search!=""?"d-block":"d-none"?>" style="justify-content: center;">
        <?php
            ...
            ?>
             <h1 class="text-center"><?= count($items) ?> Items Found!</h1><br>
                ...
                for($i=0; $i<count($items); $i++){  
                    ...
                    <div class="card mb-3 ms-3 shadow" style="width: 23.5%;">
                        ...
                        <div class="card-body" style="margin-top: -20px;">
                            ...
                            <h5 class="card-text text-dark">Rp. <?= number_format($items[$i]["Price"],0,'.','.') ?></h5>
                            <form action="editproduct.php" method="POST">
                                <button class="hoverable btn mt-2" name="editProduct" value='<?= $items[$i]["ID"] ?>'>Edit</button>
                            </form>
