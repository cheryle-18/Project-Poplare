...
<div class="container-fluid navku">
    ...
    <div class="content ps-5 pe-5 pt-5 pb-5" id="content" style="margin-top: 66px;">
        ...
        <div class="right p-2">
            <div class="title card p-2 ps-4 mb-3" style="background-color: #f5f3f0;">
                <h1><b>Order #<?= $idHtrans ?> Summary</b></h1>
            </div>
            <?php
                $jmlh = count($oDetails);
                if($jmlh>0){
                    ?>
                    <div class="itemCards p-4 w-100 rounded" style="background-color: #f5f3f0;">
                        <div class="text w-100 mb-2 ps-2" style="font-size: 1.2em;">
                            <b>Status</b> : <?= $order["status"] ?><br>
                            <b>Order Date</b> : <?= date("d/m/Y", strtotime($order["trans_date"])) ?><br>
                            <b>Total Price</b> : Rp <?= number_format($order["total"],0,'.','.') ?><br>
                            <b>Points Used</b> : <?= $order["points_used"] ?> <br>
                            <b>Points Received</b> : <?= $order["points_received"] ?> <br>
                            <b>Total Qty</b> : <?= $order["qty"] ?> products<br><br>
                            <b>Products</b> :
                        </div>
                        <table class="table table-borderless">
                        <?php
                            foreach($oDetails as $key => $value){
                                ...
                                <tr>
                                    ...
                                    <td style="width: 85%;">
                                        <div class="text ps-2 pt-1" style="font-size: 1.1em;">
                                            <b><?= $currProduct["Title"] ?></b> <br>
                        ...
                        <!-- BUTTON BACK DIGANTI INI -->
                        <div class="btns d-flex">
                            <form action="#" method="POST">
                                <button class="btn hoverable ms-2" name="back">Back</button>
                            </form>
                            <form action="usercontrol.php" method="POST">
                                <?php
                                    if($order["status"]=="Shipping"){
                                        ?>
                                            <button class="btn hoverable ms-1" name="btnArrive" value='<?= $idHtrans ?>'>I have received my order</button>
                                    <?php
                                    }
                                    else if($order["status"]=="Pending" || $order["status"]=="In Process"){
                                        ?>
                                            <button class="btn hoverable ms-1" name="btnCancel" value='<?= $idHtrans ?>'>Cancel Order</button>
                                    <?php
                                    }
                                ?>
                            </form>
