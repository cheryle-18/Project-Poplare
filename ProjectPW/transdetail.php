...
<body style="background-color: #f5f3f0;">
    ...
    <div class="content p-3 p-md-5 w-100" id="kontenku">
        <div class="card p-4">
            <h1><b>Transaction #<?= $currTrans["id_htrans"] ?></b></h1>
        </div>
        <?php
            $jmlh = count($oDetails);
            if($jmlh>0){
                ?>
                <div class="itemCards card p-4 w-100 rounded mt-3">
                    <div class="customer ps-2" style="font-size: 1.1em;">
                        <h4><b>Customer Details</b></h4>
                        <b>Name</b> : <?= $user["name"] ?> <br>
                        <b>ID</b> : <?= $user["id_user"] ?> <br>
                        <b>Email</b> : <?= $user["name"] ?> <br>
                        <b>Address</b> : <?= $shipping["address"] ?>, <?= $shipping["city"] ?>, <?= $shipping["province"] ?> <?= $shipping["postcode"] ?> <br>
                        <b>Phone Number</b> : <?= $shipping["phone"] ?> <br>
                    </div>
                    <div class="text w-100 mb-2 ps-2 mt-3" style="font-size: 1.1em;">
                        <h4><b>Order Details</b></h4>
                        <b>Status</b> : <?= $currTrans["status"] ?><br>
                        <b>Order Date</b> : <?= date("d/m/Y", strtotime($currTrans["trans_date"])) ?><br>
                        <b>Total Price</b> : Rp <?= number_format($currTrans["total"],0,'.','.') ?><br>
                        <b>Points Used</b> : <?= $currTrans["points_used"] ?> <br>
                        <b>Points Received</b> : <?= $currTrans["points_received"] ?> <br>
                        <b>Total Qty</b> : <?= $currTrans["qty"] ?> products<br><br>
                        <b>Products</b> :
                    </div>
                    ...
                    <form action="#" method="POST">
                        <button class="btn hoverable ms-2" name="btnBack" style="background-color: #013221; color: white;">Back</button>
                    </form>
                </div>
            <?php
            }
